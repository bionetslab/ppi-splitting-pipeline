...
secondary_acc_mapping = pd.read_csv(
    f"{path_to_string}/sec_ac.txt", sep="\\s+", names=["secondary_ac", "primary_ac"], skiprows=31
)
...
print(
    f"Resolving {len(lookup_with_api)} ambiguous STRING protein(s) via " f"UniProt's STRING->UniProtKB ID mapping...",
    file=sys.stderr,
)
